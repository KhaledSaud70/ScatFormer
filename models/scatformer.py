...
ScatFormer_width = {
    "L": [32, 80, 192, 384],  # 26m 83.3% 6attn
...
# ScatFormer_width = {
#     'L': [64, 128, 256, 512],  # Adjusted values
#     'S2': [64, 128, 288, 576],  # Adjusted values
#     'S1': [64, 96, 240, 448],  # Adjusted values
#     'S0': [64, 96, 192, 352],  # Adjusted values
# }
...
def stem(in_chs, out_chs, act_layer=nn.ReLU):
    ...
    return nn.Sequential(
        nn.Conv2d(in_chs, out_chs // 2, kernel_size=3, padding=1),
...
@register_model
def scatformer_s0(pretrained=False, **kwargs):
    model = ScatFormer(
...
@register_model
def scatformer_s1(pretrained=False, **kwargs):
    model = ScatFormer(
        layers=ScatFormer_depth["S1"],
        embed_dims=ScatFormer_width["S1"],
...
@register_model
def scatformer_s2(pretrained=False, **kwargs):
    model = ScatFormer(
        layers=ScatFormer_depth["S2"],
        embed_dims=ScatFormer_width["S2"],
        downsamples=[True, True, True, True],
        vit_num=4,
        drop_path_rate=0.02,
...
@register_model
def scatformer_l(pretrained=False, **kwargs):
    model = ScatFormer(
